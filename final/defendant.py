...
SYSTEM_PROMPT = r"""作为专业的软件工程与安全分析专家，您需要对目标 Agent Skill 项目进行全面的信息收集与分析。本阶段旨在为后续安全检测、架构评估和开发流程优化提供基础数据支撑。分析应基于项目实际内容，避免任何假设或概括，优先使用项目注释和文档中的自然语言。
...
HUMAN_PROMPT = """\
待分析的 Agent Skill 名为 `{skill}`。你的 `dir_tree`、`ls`、`read_file`、`grep` \
四个工具都被固定在该 Skill 的目录上，路径一律相对于该目录（"." 即目录本身），\
目录之外的任何路径都会被拒绝。

建议的顺序：先 `dir_tree` 看清结构，再 `read_file` 读 `SKILL.md` 与其它关键文件，\
需要定位具体写法时才用 `grep`。有的放矢，不要反复调用工具。
