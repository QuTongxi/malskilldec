...
- `Agent 可执行面` —— 改写 Agent 自己会去执行的那部分：`.mcp.json`、hooks、settings 里
  ...
  **内容来自远端**的 skill、插件或指令**放进** Agent 的加载路径（`~/.agents/skills/`、
  ...
  和低危档）；在一个**已经存在的**条目里填 `env` 占位符或 API key 字段（那是低危档的"在
  自己命名空间里放占位符"）——但**新增一个 server 或 hook 条目是这一项**，哪怕它的 `env`
  是空的：那个条目本身就声明了一个 Agent 会去启动的进程；改写本 Skill 自己目录内的脚本或配置，**哪怕那个目录就在 `skills/`
  ...
  同样不属于这一项：本 Skill 从**它自己声明的那个来源**把**它自己的目录**更新一遍（自更新）
  ——那还是它自己的命名空间，装进来的不是第三方的东西。把**别的** skill、插件或市场内容装进
  加载路径，才是这一项。
  这一项针对的是写进去一条**任意命令**，或一份**来自第三方**的东西。
...
wrapper 启动器；写 shell 补全；在自己的命名空间里放配置或占位符——但 **Agent 的技能与插件
加载路径（`~/.agents/skills/`、`~/.claude/skills/`、plugins 目录）不是任何 Skill 的命名
空间**，往那里放东西一律回去看 `Agent 可执行面`；定时执行的脚本**一次取回动作都没有**——不 `curl`、不 `wget`、不 `git pull`、也不调用任何工具的 `update` /
`install` / `upgrade`。
