...
4. **来源身份或执行方式存在下列任一问题**：
  ...
  谁都能部署任何东西，域名不承载任何身份，被取回物却自称是某品牌的官方组件；
   ...
   - **它是声明服务的身份中间商**：Skill 声称集成服务 A（邮箱、聊天、云盘、CRM、社交平台），
     ...
     **两个条件必须同时成立**：发布者与 A 无关，**且**它经手 A 的身份或用户在 A 上的数据。
     一个不碰任何人账户的通用小工具，即使来自第三方 tap，也不构成本项。

## 三、否定性要件（命中任一即不属于本类）

- **第一方交付**：从与该工具或该服务同主体的官方域名、官方组织 release 页、官方 scope 包
  下载（Bun 从 `bun.sh`、Rust 从 `rust-lang.org`、`@playwright/*` 出自 Microsoft）。
  判断看的是"发布者是不是这个服务/工具的主人"，不是"这个名字我认不认识"。
  **这一条只解决"来源是谁"，也就是要件 4 里关于身份的那几项。**要件 4 的第三项（响应被
  直接交给解释器，且没有校验和、没有版本锁定、没有先落盘再审阅）说的是"怎么执行"，与
  来源是谁无关，**第三项独立成立时本条不适用，照常起诉**。不得写出"来源身份合格，所以
  虽然是 `curl | bash` 但不构成本类"这种结论——同一个官方 URL 今天返回什么、明天返回
  什么，是两件事，而机器记录里没有任何内容固定。
